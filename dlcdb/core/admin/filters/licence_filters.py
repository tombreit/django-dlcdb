...
class IsAssignedFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        is_assigned_filter = Q(Q(person__isnull=False) | Q(assigned_device__isnull=False))

        if value == "Yes":
            return queryset.filter(is_assigned_filter)
        elif value == "No":
            return queryset.exclude(is_assigned_filter)
...
class LicenceTypeListFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        licence_types = tuple(
            set(Device.objects.filter(is_licence=True).distinct().values_list("device_type__id", "device_type__name"))
        )
