...
def relocate(request):
    ...
    if request.method == "POST":
        ...
        # Single-device prefill: the device detail "Move" action links here as
        # ?device=<pk>. Preselect it (scoped to the moveable set so a bogus/other
        # tenant pk is silently ignored) so the picker card renders ready to move.
        initial = {}
        prefill_pk = request.GET.get("device", "")
        if prefill_pk.isdigit():
            prefill_device = moveable_devices.filter(pk=prefill_pk).first()
            if prefill_device:
                selected_devices = [prefill_device]
                initial["devices"] = [prefill_device.pk]
        form = RelocateForm(device_queryset=moveable_devices, request=request, initial=initial)
